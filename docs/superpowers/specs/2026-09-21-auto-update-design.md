...
- **`const repo` 的改动落在切换日**，不在本设计内。新路由自己解析 can-voice 的
  release，和旧常量互不影响。
- **跟的是预发布，这是公测期的决定。** `releases/latest` **不返回预发布**，而版本号
  规则把 `YY = 0` 留给了预发布——`v27.0.1` 到 `v27.0.8` 它一个都看不见。曾经因此
  GitHub 回 404、解析器缓存 nil、清单路由永远回 204。
  现在 can-api 改成读 `/releases` 列表**按版本号挑最高的一个**（跳过草稿，不按
  列表顺序，因为 GitHub 按 `created_at` 排，一个补发的旧 tag 会排在最前面）。
  can-voice 在封闭公测，跑它的每一个人都是测试员，所以跟预发布是对的。
  **can-voice 正式发布那天必须重新决定这一条**：否则 `v28.0.x` 的预发布会被推给
  所有人，而旁边就摆着稳定的 `v27.1.x`。
- **这条路今天仍然是哑的，换了个原因。** 解析器现在够得到 `v27.0.8`，但那一版的
  21 个资产（四个产品各五种包，加上 X-Plane 插件 zip）**一个 `.sig` 都没有**——
  签名还没接。没有签名的包会被丢掉，`Builds` 是空的，请求照样落到 204。
  从 can-api 那边看，这和"GitHub 挂了"**长得一模一样**，两者都变成 204——这是
  §5"失败要安静"要的，代价是没有任何东西会说出"这条路是哑的"。
  **所以端到端那一条在签名接上之前不能测**：拿现在的包去测，看起来会像通过了。
