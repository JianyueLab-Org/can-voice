...
// flags 位。
const (
	// FlagFirst 标记一次发言的首帧：收到它就点亮 RX 指示灯并重置抖动缓冲。
	//
	// **但它不是"可以开始播放"的判据。接收端从它收到的第一个包起就播，
	// 不管那个包有没有这一位。** 首帧未必是你收到的第一个包，而且那是日常，
	// 不是边角：
	//
	//   - SUB 是全量声明、立即整体替换（router.Subscribe），所以管制员在别人
	//     说到一半时把一个频率加进台面，下一帧就投给他——那一帧没有 FlagFirst。
	//   - 一架飞机**飞进**射程也一样：投递在距离跌破 cutoff 的那一帧就开始
	//     （router 的 qualityFor），同样没有 FlagFirst。
	//
	// 等首帧的实现于是这样坏：飞行员复诵到第 8 秒，管制员把 121.800 加进台面，
	// 他一声不响，直到飞行员下一次按下 PTT——而服务端日志完全正常。
	FlagFirst uint8 = 1 << 0
	// FlagLast 标记一次发言的尾帧：收到它就可以**立刻**熄灭 RX 指示灯，
	// 不必等静默超时走完——那会让指示灯在松开 PTT 后多亮半秒。
	//
	// **但它是尽力而为的优化，不是熄灯的机制。接收端必须同时跑一个静默超时。**
	// 两条理由，第二条是设计使然而不是运气：它坐在 QUIC 的不可靠数据报上，会丢；
	// 而服务端会在一次发言中途停止向某个听众投递（对方飞出了射程）且不发任何
	// 通知（见下面 Seq 那一段的最后一条），那种情况下尾帧**保证**到不了。
...
type Header struct {
	...
	//     一次发言结束到下一次开始之间不清零，**不说话的时候也不走**（没有帧，
	//     就没有号）。起始值无所谓：接收端只看差值，从它收到的**第一个包**起算
	//     ——第一个包未必带 FlagFirst，见上面 FlagFirst 那一段。按发言重置在
	//     不可靠数据报上是错的：首帧本来就可能丢，丢了之后接收端看到的是一个
	//     **倒退**的序号，和"一个很旧的乱序包"分不开，于是整段新发言会被当成
	//     过期的丢掉。
	//   - **一个音频帧只有一个序号，而且同一个频率上只会送到一份。** 一次 PTT
	//     同时发在多个频率上时，客户端给**每个** TX 频率各发一份上行，那几份
	//     带的是**同一个** Seq；服务端做交叉耦合扇出时也照抄，不按频率另起一个。
	//     服务端还会把耦合拷贝里**发言者自己也在发射的那个频率**扣掉：那一份他
	//     自己已经直发了，再补一份就是同一个键上两个同号的包。于是接收端按
	//     (Speaker, FreqKHz) 分流之后看到的确实是一串连号，而不是 8, 8, 9, 9。
	//
	//     **这一条对客户端是承重的：声明了 XC(A,B) 就必须真的在 A 和 B 上各发
	//     一份。** 耦合只有在两个频率都在你的 TX 授权集合里时才生效，所以"声明
	//     了耦合"蕴含"两边都能发"；服务端不会替你把音频搬到你自己也在发射的那个
	//     频率上去。只发一份的客户端在另一个频率上是哑的。
	//   - **同时订阅一对耦合频率两边的人，会在两边各收到一份。这是有意的，
	//     不要"修"掉它。** 两份带同一个 Seq，但落在两个不同的 (Speaker, FreqKHz)
	//     键上，是两条独立的流、两个独立的缓冲区——一个 RX 开在两行上的无线电栈
	//     本来就该在两行上都听到那次耦合发言。跨频率去重会让其中一行永远是哑的。
	//   - **按 (Speaker, FreqKHz) 建缓冲。** Seq 是发言者自己的计数器，
	//     两个发言者的序号空间毫无关系，服务端也不重新编号。
	//   - **比较要用 16 位回绕算术**（RFC 1982 那一套）：`(b-a) mod 2¹⁶ < 2¹⁵`
	//     就认为 b 在 a 之后。有两处与 RFC 1982 不一致，照这里写，不要照 RFC 写：
	//       - **b == a 时这个式子给"在后面"**（0 < 32768 为真）。所以**先判相等、
	//         再判先后**，否则一个重复帧会被当成新帧收下。
	//       - **差值恰好是 2¹⁵ 时 RFC 1982 说"未定义"**，这里定成**不在后面**
	//         （式子本身就给这个答案），也就是丢弃那一侧。半个序号空间是 10.9
	//         分钟的连续音频，抖动缓冲区的深度离它有三个数量级，实际流量里到不了
	//         这一点；写死它只是为了两个实现不会在这里分岔。
	//     20 毫秒一帧时 uint16 每 65536 帧、约 21.8 分钟回绕一次，而那是**连续
	//     发话**的 21.8 分钟——直接比大小的实现会在那一刻把整条流卡住，直到序号
	//     重新追上来。
	//   - **发言之内的空档才是丢包。** FlagFirst 到 FlagLast 之间少了 n 个号就是
	//     丢了 n 帧；跨发言的跳变不是丢包（中间那些帧可能发在别的频率上），
	//     所以收到 FlagFirst 是重置缓冲，而不是去补空档。
	//   - **发言之内的空档也不一定是网络丢的。** 服务端会在一次发言中途停止向
	//     某个听众投递（对方飞出了射程），那在接收端看来和丢包一模一样。
	//     不必区分：两种都只能交给 PLC，而服务端不会为此发任何通知——这也正是
	//     FlagLast 保证不到达的那种情况，所以熄灯要靠超时。
	//
	// 服务端这一半各自有钉子：原样转发与耦合拷贝同号由 router 的
	// TestFanoutRelaysTheSequenceNumberUntouchedOnEveryCoupledCopy；扣掉发言者
	// 自己也在发的那个频率、以及由此得到的"一个键上一串连号"由
	// TestACoupledCopySkipsAFrequencyTheSpeakerAlsoTransmitsOn 与
	// TestASinglePTTOnCoupledFrequenciesLeavesAConsecutiveSeqRunOnEachOne；
	// 订阅两边各收一份由 TestAListenerOnBothCoupledFrequenciesHearsItOnBothRows；
	// 中途加入的听众下一帧就开始收由 router 的
	// TestAFrequencyAddedMidTransmissionStartsReceivingOnTheNextFrame 和
	// TestAListenerComingIntoRangeMidTransmissionStartsReceivingThere。
	Seq uint16
