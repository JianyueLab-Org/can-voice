...
| `refused` | 这条消息不该出现在这里（比如 HELLO 之前先发了别的）、`follow` 不是一个合法呼号，或者这位成员的 rating 不够（< 1，和 can-api 的 `/api/v1/public/auth` 同一条规则） | 同上。换 token 没有用——rating 不够的成员换多少张票都一样，他该去考试，而那是服务端说不出口的事 |
