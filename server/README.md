...
**上行有速率上限，超出的帧被丢弃。** 服务端按会话记一个令牌桶：速率是
`50 帧/秒 × 授权后的 max_tx × 1.5`，桶的容量是 `25 帧 × 授权后的 max_tx`。50 来自
20 毫秒一帧；乘 `max_tx` 是因为一次 PTT 在每个 TX 频率上各发一份（见上面 `seq` 那几条）；
1.5 的余量留给客户端声卡时钟和服务端时钟之间的那点差值——ATIS 席位一播就是几天，
余量取 1.0 的话桶会慢慢见底，一条完全正常的通播开始断断续续。

超额的包**在解包头之前**就被丢掉，**连接不断**，会话继续，桶自己回血。服务端回一条
`kind` 为 `rate_limited` 的 NOTICE，同一条会话最多 3 秒一条，第一条不等。按正常速率
发话的客户端碰不到这条上限；碰到了就是客户端的发包循环出了故障（PTT 卡住是典型），
不丢的话，那一路上行会把同频率上每个听众队列里的正常语音挤掉。
...
机队对关闭码 2 的遵守方式和桌面端**不一样**，写在这里免得下一个人以为它在违反
上面那张表：它没有用户可以告诉，真停下就没有人把这个席位拉回来，所以它退到最大
退避（一分钟）再试一次，并单独记一条 warn 说明这个账号在别处被用着。要紧的那一半
——**不再一秒就重连**——是照做的。机队自己的两条播出约定（嗓子是 can-audio 那两个
男声、先中文后英文）记在仓库根 `README.md` 的《服务端通播机器人》一节。
...
操作清单（密钥、证书、compose、防火墙、切流时机、客户端发版）见
[`docs/deploy.md`](../docs/deploy.md)。

下面五步是同一件事的细部。**第一步一辈子做一次**（换密钥就要 can-api 和这边
同时改，在两边都改完之前全网连不上），其余四步是换台机器就重做一遍的例行动作。
...
- **某个人说话别人听着断断续续，日志里是 `uplink audio is over the rate limit`**：
  那条上行超过了 `50 帧/秒 × max_tx`（见"数据面包头"）。这是客户端的发包循环出了
  故障，不是网络——PTT 卡住是典型。服务端只丢多出来的帧、不断连接，客户端回到正常
  速率之后自己就好；那条会话同时会收到 `rate_limited` 的 NOTICE。
