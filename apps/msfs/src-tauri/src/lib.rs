...
/// 收起来的是连接卡片，横幅不收）。
///
/// **但 msfs 比 xpc 多占一块，这一条算进去了才算数。** 横幅不收，而 msfs 的
/// `sim_problem` 横幅正是不收的那一叠里的一条：MSFS 没开的时候它就在，文案是
/// `连不上 MSFS：{detail}`，`{detail}` 是 SimConnect 原样吐出来的英文，**没有长度
/// 上限**，也没有 `truncate`。在 320×220 这个下限上，头部、折行的无线电那一行和
/// 状态栏之后剩不下多少高度，而消息卡片是 `flex-1 min-h-0`——被挤掉的是它，不是
/// 横幅。460×340 排得下，320×220 排不下还得看 detail 多长。
///
/// 这不是这次改布局带来的：b20a092 之前 `sim_problem` 也是一直在的，只是一行
/// 跟着头部折行的 span，现在多的只是边框和内边距。写在这里是因为下一个来动这
/// 一对数字的人会看这段注释，而「两个端一模一样」这句话单看是会骗人的。
